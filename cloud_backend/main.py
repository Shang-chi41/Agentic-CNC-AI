...
from cloud_backend.services.auth_service import seed_default_users, validate_auth_config
...
from cloud_backend.routes.pose_routes      import router as pose_router
from cloud_backend.routes.system_routes    import router as system_router
...
def health():
    """Process liveness only; does not claim dependencies are ready."""
    return {"status": "ok", "service": "cloud_backend"}


@app.get("/ready", tags=["System"])
def ready():
    """Readiness for traffic: MongoDB is a mandatory Cloud dependency."""
    if not mongo_ping():
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="MongoDB unavailable",
        )
    return {"status": "ready", "service": "cloud_backend", "mongodb": "ok"}
...
app.include_router(pose_router, tags=["Realtime Pose"])
...
@app.get("/guide", tags=["Pages"], include_in_schema=False)
def page_guide():
    return _serve_page("operator_guide.html")
