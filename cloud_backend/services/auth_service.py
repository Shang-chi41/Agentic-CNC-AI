...
Bootstrap user khi collection còn trống:
  - Yêu cầu BOOTSTRAP_ADMIN_USERNAME + BOOTSTRAP_ADMIN_PASSWORD.
  - Không có tài khoản mặc định hard-code.
...
from cloud_backend.services.auth_bootstrap_policy import resolve_bootstrap_users
...
def seed_default_users() -> None:
    """Bootstrap Users collection without silently creating known credentials.

    An empty Users collection requires an explicit one-time bootstrap admin
    through environment variables. Known default accounts are never auto-created.
    """
    col = get_col(_USERS_COL)
    if col.count_documents({}) > 0:
        return

    users, mode = resolve_bootstrap_users(
    ...
    logger.info("Đã bootstrap %d user (%s)", len(users), mode)
