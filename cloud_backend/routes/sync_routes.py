...
from cloud_backend.runtime.command_recovery_policy import stale_command_recovery_policy
from cloud_backend.runtime.gcode_sync_policy import merge_edge_gcode_record
...
def _verify_sync_key(key: str | None = Depends(_SYNC_KEY_HEADER)) -> str:
    expected = os.getenv(_SYNC_KEY_ENV, "").strip()
    insecure = {"", "CHANGE_ME", "CHANGE_ME_GENERATE_A_LONG_RANDOM_SYNC_KEY", "cnc_edge_sync_key_change_in_production"}
    if expected in insecure:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="SYNC_API_KEY chưa được cấu hình an toàn trên Cloud Backend",
        )
...
def _stale_command_recovery_policy(action: str) -> str:
    """Compatibility/test hook around the deterministic recovery policy."""
    return stale_command_recovery_policy(action)


def _merge_edge_gcode_record(
    existing: dict[str, Any] | None,
    incoming: dict[str, Any],
    *,
    synced_at: str,
) -> dict[str, Any]:
    """Compatibility/test hook around the deterministic G-code merge policy."""
    return merge_edge_gcode_record(existing, incoming, synced_at=synced_at)
...
def _simulation_phase_collection(record: dict[str, Any]) -> str | None:
    phase = str(record.get("simulation_phase") or record.get("dataflow_mode") or "").lower()
    if phase == "pre_run_check":
        return "Simulation_Check_Data"
    if phase == "parallel_run_simulation":
        return "Simulation_Parallel_Run"
    if phase == "machine_run":
        return "Simulation_Machine_Run"
    if phase == "connectivity_test":
        return "Simulation_Connectivity_Test"
    return None


def _gcode_run_command_ready(gcode_id: str, command_doc: dict[str, Any]) -> tuple[bool, list[str]]:
    reasons: list[str] = []
    params = command_doc.get("params") or {}
    auth = params.get("run_authorization") or {}
    if not auth.get("authorized"):
        reasons.append("Command thiếu run_authorization.authorized=true")

    try:
        gdoc = get_col(_COL_GCODE).find_one({"_id": ObjectId(gcode_id)})
    except Exception:
        return False, ["gcode_id không hợp lệ"]

    if not gdoc:
        return False, ["Không tìm thấy G-code"]

    status = str(gdoc.get("status", "")).lower()
    if status != "queued":
        reasons.append(f"G-code không ở trạng thái queued trước khi Edge claim (status='{gdoc.get('status')}')")

    if gdoc.get("machine_run_authorized") is not True:
        reasons.append("G-code chưa có machine_run_authorized=true")

    if gdoc.get("confirmed_from_status") != "approved":
        reasons.append("G-code không được confirm từ trạng thái approved")

    if gdoc.get("checksum") and gdoc.get("approved_checksum"):
        if gdoc.get("checksum") != gdoc.get("approved_checksum"):
            reasons.append("Checksum khác với bản đã CHECK/approved")
...
# ══════════════════════════════════════════════════════════════════════════
#  UNIFIED RUNTIME STATUS SYNC (Mission 05B)
# ══════════════════════════════════════════════════════════════════════════

class RuntimeStatusSyncRequest(BaseModel):
    records: list[dict[str, Any]]


@router.post("/runtime_status", summary="Edge push unified runtime/system status")
def sync_runtime_status(body: RuntimeStatusSyncRequest, _: str = SyncAuth) -> dict:
    """Upsert Edge runtime heartbeat by runtime_id.
    ...
    col = get_col(_COL_RUNTIME)
    now = _now_str()
    count = 0
    for record in body.records:
        runtime_id = str(record.get("runtime_id") or f"edge:{record.get('entrypoint', 'unknown')}")
        doc = {k: v for k, v in record.items() if k not in ("_id",)}
        doc.update({"runtime_id": runtime_id, "synced_at": now})
        result = col.update_one({"runtime_id": runtime_id}, {"$set": doc}, upsert=True)
        count += int(result.upserted_id is not None or result.modified_count > 0)
...
def sync_simulation(body: SimSyncRequest, _: str = SyncAuth) -> dict:
    ...
    for record in body.records:
        ...
        if edge_id:
            ...
            result = col.update_one(target, {"$set": doc}, upsert=True)
            count += int(existing is None or result.modified_count > 0)
            phase_col = _simulation_phase_collection(doc)
            if phase_col:
                get_col(phase_col).update_one(target, {"$set": doc}, upsert=True)
        else:
            result = col.insert_one(doc)
            phase_col = _simulation_phase_collection(doc)
            if phase_col:
                phase_doc = dict(doc)
                phase_doc["edge_simulation_id"] = edge_id or str(result.inserted_id)
                get_col(phase_col).insert_one(phase_doc)
            count += 1
    return {"status": "ok", "upserted": count}
...
def sync_gcodes(body: GCodeSyncRequest, _: str = SyncAuth) -> dict:
    ...
    for record in body.records:
        ...
        if edge_id:
            ...
            doc["edge_gcode_id"] = edge_id
            merged = _merge_edge_gcode_record(existing, doc, synced_at=now)
            result = col.update_one(target, {"$set": merged}, upsert=True)
            count += int(existing is None or result.modified_count > 0)
        else:
            doc["synced_at"] = now
            col.insert_one(doc)
...
def poll_commands(_: str = SyncAuth) -> list[dict]:
    ...
    # Lost ACK is not proof that a physical side effect did not happen.
    # Only emergency hold/stop are safe to repeat automatically.  HOME/JOG/
    # RESUME/UNLOCK/unknown commands are quarantined for reconciliation.
    stale_query = {
        "status": "processing",
        "action": {"$ne": "run_gcode"},
        "$or": [
    ...
    for stale in list(col.find(stale_query)):
        action = str(stale.get("action") or "").strip().lower()
        policy = _stale_command_recovery_policy(action)
        if policy == "retry_safe":
            ...
            update = {
                ...
                "recovered_at": now.isoformat(),
                "recovery_policy": "reconciliation_required",
                "error": (
                    f"Command '{action or 'unknown'}' mất ACK khi processing; "
                    "không tự chạy lại vì side effect vật lý chưa xác định"
                ),
            }
        col.update_one({"_id": stale["_id"], "status": "processing"}, {"$set": update})
    ...
    for _ in range(10):
        ...
        if doc.get("action") == "run_gcode":
            ...
            ready, reasons = _gcode_run_command_ready(gcode_id, doc)
            if not ready:
                col.update_one(
                    {"_id": doc["_id"]},
                    {"$set": {
                        "status": "failed",
                        "error": "RUN bị khóa trước khi Edge claim: " + "; ".join(reasons),
                        "executed_at": now.isoformat(),
                    }},
                )
                if gcode_id:
                    try:
                        get_col(_COL_GCODE).update_one(
                            ...
                            {"$set": {
                                ...
                                "blocked_reason": "RUN bị khóa trước khi Edge claim: " + "; ".join(reasons),
                            }},
                        )
                    except Exception:
                        pass
