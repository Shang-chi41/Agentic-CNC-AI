...
def _normalize_sensor(doc: dict) -> dict:
    ...
    def _axis(name: str) -> dict:
        a = axes.get(name, {}) if isinstance(axes, dict) else {}
...
def _normalize_simulation(doc: dict) -> dict:
    ...
    edge = d.get("edge_computing", {})
    if not isinstance(edge, dict):
        edge = {}
    health = d.get("system_health", edge.get("system_health", {}))
...
def connection_status(user: CurrentUser) -> dict:
    ...
    - matlab  : bản ghi Simulation_Data gần đây do main.py/MATLAB operational runtime
                tính toán thật (`pre_run_check`, `parallel_run_simulation`,
                `machine_run`); không tính `connectivity_test` vì đó là main_sim_only.
    - sim_test: bản ghi Simulation_Data gần đây có `simulation_phase=connectivity_test`
                từ main_sim_only; dùng để test kết nối, không mở quyền chạy G-code.
    - nxmcd   : bản ghi Simulation_Data gần đây có `drives_nx_mcd=True`
    ...
    Response shape::

        {
            ...
            "sim_test":{ "connected": false, "status": "no_data", "age_s": null },
    ...
    conn = system.get("connection", {})
    # Backward-compatible response for existing frontend code, but backed by
    # the unified source-of-truth contract.
    return {
        "fluidnc": conn.get("fluidnc", {}),
        "matlab": conn.get("matlab_main", {}),
        "matlab_main": conn.get("matlab_main", {}),
        "nxmcd": conn.get("nxmcd", {}),
        "sim_test": conn.get("sim_test", {}),
        "mongodb": conn.get("mongodb", {}),
    }
    ...
    # MATLAB MAIN — main.py operational modes only. main_sim_only/connectivity_test
    # is deliberately excluded so the HMI does not show "MATLAB MAIN online"
    # merely because a connectivity test packet exists.
    sim_col = get_col(_COL_SIM)
    sim_doc = sim_col.find_one(
        {"simulation_phase": {"$in": ["pre_run_check", "parallel_run_simulation", "machine_run"]}},
        sort=[("timestamp", -1)],
    )
    matlab = _result(_age(sim_doc, "timestamp", "created_at"), _STALE_SIMULATION)

    # SIM TEST — main_sim_only / connectivity-test only. This proves test
    # connectivity, not machine-run readiness and not MATLAB MAIN readiness.
    sim_test_doc = sim_col.find_one(
        {"simulation_phase": "connectivity_test"},
        sort=[("timestamp", -1)],
    )
    sim_test = _result(_age(sim_test_doc, "timestamp", "created_at"), _STALE_SIMULATION)

    # NX MCD — any recent packet explicitly marked as driving NX MCD.
    nxmcd_doc = sim_col.find_one(
        {"drives_nx_mcd": True},
        sort=[("timestamp", -1)],
    )
    nxmcd = _result(_age(nxmcd_doc, "timestamp", "created_at"), _STALE_SYNC)

    # MongoDB Atlas — ping trực tiếp
    mongo_ok = mongo_ping()
    mongodb  = {"connected": mongo_ok, "status": "online" if mongo_ok else "offline", "age_s": None}

    return {"fluidnc": fluidnc, "matlab": matlab, "nxmcd": nxmcd, "sim_test": sim_test, "mongodb": mongodb}