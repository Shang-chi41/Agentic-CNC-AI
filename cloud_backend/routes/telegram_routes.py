...
from cloud_backend.middleware.auth import OperatorUser
from cloud_backend.services.mongo_service import docs_to_list, get_col
from cloud_backend.services.telegram_security import verify_webhook_secret
...
def _verify_telegram(request_body: bytes, x_telegram_token: str) -> bool:
    """Fail-closed verification for Telegram's configured webhook secret token.

    ``request_body`` is intentionally unused: Telegram sends the configured
    secret token directly in ``X-Telegram-Bot-Api-Secret-Token``.
    """
    del request_body
    return verify_webhook_secret(
...
async def telegram_webhook(request: Request) -> dict:
    ...
    # Bảo mật: remote machine-control qua Telegram bị vô hiệu nếu chưa pin owner chat.
    owner_chat_id = _configured_chat_id()
    if not owner_chat_id:
        raise HTTPException(status_code=503, detail="Telegram machine-control chưa cấu hình TELEGRAM_CHAT_ID")
...
async def notify_alarm(body: AlarmNotifyRequest, user: OperatorUser) -> dict:
...
def telegram_bot_status() -> dict:
    ...
    return {
        ...
        "has_chat_id": bool(_configured_chat_id()),
        "has_webhook_secret": bool(os.getenv("TELEGRAM_WEBHOOK_SECRET", "").strip()),
