...
def get_network_status(user: CurrentUser) -> dict:
    ...
    edge_online = False
    edge_entrypoint = "none"
    last_sync_str = None
    try:
        from cloud_backend.routes.system_routes import build_system_status
        system = build_system_status()
        edge = (system.get("connection") or {}).get("edge_runtime") or {}
        edge_online = bool(edge.get("connected"))
        edge_entrypoint = str(edge.get("active_entrypoint") or "none")
    except Exception:
        pass
    ...
    return {
        ...
        "edge_entrypoint":  edge_entrypoint,
