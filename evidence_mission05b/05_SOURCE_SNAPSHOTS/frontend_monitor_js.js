...
function deriveDataflowMode(payload = {}) {
    const mode = payload.dataflow_mode || payload.simulation_phase || '';
    if (mode) return mode;
    const src = payload.source || payload.control_owner || 'unknown';
    const state = String(payload.state || '').toUpperCase();
    if (payload.test_only || payload.simulation_phase === 'connectivity_test' || src === 'connectivity_test') return 'connectivity_test';
    if (src === 'matlab_check' || state === 'CHECK') return 'pre_run_check';
    if (src === 'stream_fluidnc') return 'machine_run';
    if (src === 'idle_fluidnc') return 'manual_jog_or_direct_line';
    return 'idle';
}

function modeLabel(mode) {
    switch (mode) {
        case 'pre_run_check': return 'PRE-RUN CHECK';
        case 'machine_run': return 'MACHINE RUN';
        case 'manual_jog_or_direct_line': return 'JOG / DIRECT LINE';
        case 'connectivity_test': return 'CONNECTIVITY TEST';
        case 'home_sync': return 'HOME SYNC';
        case 'estop': return 'ESTOP / COLLISION';
        default: return 'IDLE / UNKNOWN';
    }
}

function updateModeOvals(mode, payload = {}) {
    const active = mode || 'idle';
    document.querySelectorAll('[data-mode-pill]').forEach(el => {
        const thisMode = el.getAttribute('data-mode-pill');
        el.classList.remove('active', 'warn', 'alarm');
        if (thisMode === active) {
            if (payload.collision || String(payload.state || '').toUpperCase().includes('ALARM') || active === 'estop') el.classList.add('alarm');
            else if (active === 'pre_run_check' || active === 'connectivity_test') el.classList.add('warn');
...
function updateSyncGate(payload = {}) {
    ...
    const mode = deriveDataflowMode(payload);

    _setText('dtModeLabel', modeLabel(mode));
    _setText('dtControlOwner', owner);
    _setText('dtSourceTruth', payload.source_of_truth || (mode === 'pre_run_check' ? 'matlab_simulink' : mode === 'machine_run' || mode === 'manual_jog_or_direct_line' ? 'fluidnc_mpos' : '---'));
    _setText('dtSyncEpoch', payload.sync_epoch_id || '---');
    _setText('dtSyncEpochInline', payload.sync_epoch_id || '---');
    _setText('dtPoseSource', payload.source || owner);
    updateModeOvals(mode, payload);
...
function initMetricTabs() {
    const tabs = Array.from(document.querySelectorAll('[data-metric-tab]'));
    const panels = Array.from(document.querySelectorAll('[data-metric-panel]'));
    if (!tabs.length || !panels.length) return;

    const activate = (name) => {
        tabs.forEach(tab => {
            const isActive = tab.dataset.metricTab === name;
            tab.classList.toggle('active', isActive);
            tab.setAttribute('aria-selected', String(isActive));
        });
        panels.forEach(panel => {
            const isActive = panel.dataset.metricPanel === name;
            panel.classList.toggle('active', isActive);
            panel.hidden = !isActive;
        });
    };

    tabs.forEach(tab => tab.addEventListener('click', () => activate(tab.dataset.metricTab)));
    activate('mpos');
}
...
function _connConnected(value) {
    if (value && typeof value === 'object') return Boolean(value.connected);
    return value === true || value === 'connected' || value === 'online';
}

function _setSmallDot(id, value) {
    const el = _el(id);
    if (!el) return;
    el.className = 'sdot ' + (_connConnected(value) ? 'on' : 'off');
}

async function fetchConnectionStatus() {
    try {
        const system = await api.get('/api/system/status');
        const data = system?.connection || {};
        _setSmallDot('netEdgeDot', data.fluidnc);
        _setSmallDot('netMatlabDot', data.matlab_main);
        _setSmallDot('netSimTestDot', data.sim_test);
        _setSmallDot('netNxDot', data.nxmcd);
        _setSmallDot('netMongoDot', data.mongodb);
    } catch (_) {
        ['netEdgeDot', 'netMatlabDot', 'netSimTestDot', 'netNxDot', 'netMongoDot'].forEach(id => {
            const el = _el(id);
            if (el) el.className = 'sdot off';
        });
    }
}
